...
// let any special processing the pinmap needs happen
  ...
  // say hello
  VLF("");
  VF("MSG: OnStepX, version ");
  V(FirmwareVersionMajor);
  V(".");
  V(FirmwareVersionMinor);
  VL(FirmwareVersionPatch);
  VF("MSG: OnStepX, MCU ");
  VLF(MCU_STR);
  VF("MSG: OnStepX, pinmap ");
  VLF(PINMAP_STR);
  ...
  VF("MSG: System, start input sense service task (rate ");
  V(pollingRate);
  VF("ms priority 7)... ");
  if (tasks.add(pollingRate, 0, true, 7, sensesPoll, "SysSens")) {
    VLF("success");
  } else {
    VLF("FAILED!");
  }
